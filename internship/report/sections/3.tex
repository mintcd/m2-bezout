...
\begin{definition}[BST-like distribution for regexes]
  ...
  \begin{enumerate}
    ...
    \item $p\left(\vcenter{\hbox{\begin{tikzpicture}[level distance=8mm, sibling distance=7mm, edge from parent/.style={draw, shorten <=1mm}]
...
\begin{equation}
  ...
  Y_{n+2} \stackrel{d}{=} I_n\left(Y_{U_n}^{(1)} + Y_{n+1-U_n}^{(2)} + 1\right) + J_n\max\left(Y_{U_n}^{(1)}, Y_{n+1-U_n}^{(2)}\right).
\end{equation}

In (\ref{eq:yield_indicator}), $(I_n, J_n)$ is a pair of mutually exclusive Bernoulli random variables with $\mathbb{E}[I_n] = u$ and $\mathbb{E}[J_n] = v$ such that $I_n+J_n\le 1$. The sequences $\bm{Y}^{(1)} = (Y_n^{(1)})_{n \ge 1}$ and $\bm{Y}^{(2)} = (Y_n^{(2)})_{n \ge 1}$ are independent copies of the sequence $\bm{Y} = (Y_n)_{n \ge 1}$. The notion of equality is in distribution. Note that we can reuse the same variables $Y_{U_n}^{(1)}, Y_{n+1-U_n}^{(2)}$ and $U_n$ across both terms because at most one indicator is non-zero for any given realization. Let $\bm{I} = (I_n)_{n \ge 1}$, and define $\bm{J}$ and $\bm{U}$ similarly. We assume that the sequence of pairs $(\bm{I}, \bm{J})$ and the sequence $\bm{U}$ each consist of independent and identically distributed elements. Furthermore, the entire stochastic processes $(\bm{I}, \bm{J})$, $\bm{U}$, $\bm{Y}^{(1)}$, and $\bm{Y}^{(2)}$ are mutually independent.

\subsection{Lower and Upper Bounds of the Expectation}
...
For an upper bound, we use the fact that $\max(Y_i, Y_{n+1-i}) \le Y_i + Y_{n+1-i}$, which gives an upper bound sequence
\begin{equation}
...
\begin{equation}
  ...
  y_{n+2}^{L} = u + \dfrac{2u+v}{n}\sum_{i=1}^{n}y_i^{L} + \dfrac{v}{2n}\sum_{i=1}^{n}|y_i^{L} - y_{n+1-i}^{L}|, \quad y_1^{L}=y_2^{L}=0.
\end{equation}


\begin{remark}
  Another lower bound is obtained as follows. By Jensen's inequality
  $$\EE[\max(Y_i, Y_{n+1-i})] \ge \max(\EE[Y_i], \EE[Y_{n+1-i}]) = \max(y_i, y_{n+1-i}) = y_{\max(i, n+1-i)},$$
  where the last equality holds because $y_n$ is non-decreasing in $n$ \textit{(we have to prove it)}. This gives the following lower bound sequence
  \begin{align}
    y_{n+2}^{L} & = u + \dfrac{2u}{n}\sum_{i=1}^{n}y_i^{L} + \dfrac{2v}{n}\sum_{i=\lfloor (n+1)/2 \rfloor}^{n}y_{i}^{L}, & y_1^{L}=y_2^{L}=0.
  \end{align}
\end{remark}

Looking at the bounds (\ref{eq:lower_bound}), we know that when $2u+v > 1$, $(y_n)$ grows at least polynomially.
