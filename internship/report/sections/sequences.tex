\begin{center}
  \vspace{1em}
  \textbf{\Large Asymptotic Analysis of Some Sequences}
  \vspace{1em}
\end{center}
...
\begin{proof}
  ...
  \begin{itemize}
    \item If $n=2k$, then $S_{\lfloor n/2\rfloor} - S_{\lfloor (n-1)/2\rfloor} = x_k$ and $m_n - m_{n-1} = -x_{k}$. The right-hand side is $(a+b)x_n - bx_k + c$.
    \item If $n=2k+1$, then $S_{\lfloor n/2\rfloor} - S_{\lfloor (n-1)/2\rfloor} = 0$ and $m_n - m_{n-1} = x_{k+1}$. The right-hand side is $(a+b)x_n - bx_{k+1} + c$.
  \end{itemize}

  Collecting the two cases, we have
  \begin{align*}
    nx_{n+2} - (n-1)x_{n+1} = (a+b)x_n - b x_{\lceil n/2\rceil} + c.
  \end{align*}
  Therefore,
  \begin{align*}
     & n(x_{n+2} - x_{n+1})                                                                                                                              \\
     & = -x_{n+1} +(a+b)x_n - b x_{\lceil n/2\rceil} + c                                                                                                 \\
     & = -\dfrac{n-2}{n-1}x_n - \dfrac{a+b}{n-1}x_{n-1} + \dfrac{b}{n-1} x_{\lceil (n-1)/2\rceil} +(a+b)x_n - b x_{\lceil n/2\rceil} + \dfrac{n-2}{n-1}c \\
     & =  \dfrac{a+b}{n-1}(x_n - x_{n-1}) + \dfrac{n-2}{n-1}(a+b-1)x_n +  + c
  \end{align*}
\end{proof}
...
We guess the asymptotic of $(x_n)$, then do experiments, then try to prove it. Suppose $x_n \sim D n^{\alpha}$.
