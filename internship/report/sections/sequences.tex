...
\begin{equation}
  ...
  x_{n+2} = \dfrac{a}{n}\sum_{i=1}^{n}x_i + \dfrac{b}{n}\sum_{i=1}^{n}|x_i - x_{n+1-i}| + c, \quad n \ge 1, \quad x_1=x_2=0 \quad (a>1, 0 < b < 1, c>0)
\end{equation}
\begin{equation}
  \label{eq:recurrence-sum-half-sum}
  x_{n} = \dfrac{2u}{n}\sum_{i=0}^{n-1}x_i + \dfrac{2v}{n}\sum_{i=\lfloor (n-1)/2 \rfloor}^{n-1}x_{i}, x_0=x_1=1 \quad (u,v > 0, u+v\le1, 2u+v>1).
\end{equation}
...
\begin{proposition}
  Consider the sequence defined by \eqref{eq:recurrence-sum-half-sum}. We have $x_n = \Theta(n^{\alpha})$, where $\alpha$ is the unique solution of
...
\begin{proof}
  ...
  \begin{align*}
    \dfrac{x_{n}}{K}
     ...
     & \ge \dfrac{2u}{n}\left(\left(\int_{0}^{n-1}x^{\alpha}\,\mathrm{d}x\right)+n\right)  + \dfrac{2v}{n}\left(\int_{(n-2)/2}^{n-1}x^{\alpha}+\dfrac{n+1}{2}\right) \,\mathrm{d}x \\
     & = \dfrac{2u}{n}\left(\dfrac{(n-1)^{\alpha+1}}{\alpha+1} + n\right) + \dfrac{2v}{n}\left(\dfrac{(n-1)^{\alpha+1} - (n/2)^{\alpha+1}}{\alpha+1} + \dfrac{n+1}{2}\right)       \\
     & = \dfrac{(n-1)^{\alpha+1}}{n(\alpha+1)}\left(2u+2v - v2^{-\alpha}\right)+ 2u + v\left(1+\dfrac{1}{n}\right)                                                                 \\
  ...
  Let $N_0 = \left\lceil \left(\dfrac{2u + v - 1}{\alpha+1}\right)^{\frac{1}{\alpha-1}} \right\rceil$. The inequality holds for every $n\ge N_0$. Now we choose $K$ for it to hold for $n < N_0$. In particular,

  $$K = \min\left(\dfrac{1}{2}, \min\limits_{3\le i < N_0}\dfrac{x_i}{i^{\alpha}}\right)$$
